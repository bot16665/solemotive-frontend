...
const CustomAlert = ({ message, type, onClose }) => (
  <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50">
    <div className={`${type === "success" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
...
const ShoeDetails = () => {
  ...
  const handleMouseMove = (e) => {
    ...
    const walk = (x - startX) * 0.1; // Adjust scroll speed
    ...
    <div className="min-h-screen bg-[#dfd86b] pt-12 md:pt-20">
      ...
      <div className="container mx-auto p-4 md:p-8">
        <div className="flex flex-col md:flex-row">
          ...
          <div
            ref={containerRef}
            className="md:hidden w-full overflow-x-auto flex gap-4 mb-6 cursor-grab scroll-smooth snap-x snap-mandatory"
          ...
          >
            {[shoe.image, ...(shoe.thumbnails || [])].map((image, index) => (
              <img
                ...
                className="w-full h-auto max-h-[400px] object-contain rounded-lg flex-shrink-0 snap-center"
              />
            ))}
          </div>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-3">
                {["UK6", "UK7", "UK8", "UK9", "UK10"].map((size) => (
                  <button
                    key={size}
                    className={`px-4 py-2 md:px-6 md:py-3 border text-base md:text-lg font-semibold rounded-lg ${selectedSize === size ? "bg-black text-white" : ""
                      }`}
